...
export default class Controller {
    ...
    private alreadyDidSinglePress: {[Key in BUTTON]: boolean} = {
        LEFT: false,
        RIGHT: false,
        DOWN: false,
        A: false,
     ...
     */
    update(): void {
        this.controller = [...navigator.getGamepads()].filter(gp => gp?.id?.includes("Xbox"))[0];
        const now = Date.now();
    ...
    wasPressed(button: BUTTON): boolean {
        ...
        if (this.debounceTimes[button][0] == 0) {
        ...
        else if (this.lastPressedTime[button] > this.lastReleasedTime[button]) {
            // the button is being pressed or held down
            if (now - this.lastReleasedTime[button] > this.debounceTimes[button][0]) {
                // button has been pressed for the first time
                // decide whether to start repeating
                if ((now - this.lastReleasedTime[button] - this.debounceTimes[button][0]) > this.debounceTimes[button][1]) {
                    // this.lastReleasedTime[button] += this.debounceTimes[button][1];
                    return true;
                }
            } else if (!this.alreadyDidSinglePress[button]) {
                // button is being held, for less than the single press cooldown, and we have yet to emit a single press 
                this.alreadyDidSinglePress[button] = true;
                return true;
            } else {
            ...
            this.alreadyDidSinglePress[button] = false;
        }
    }
};